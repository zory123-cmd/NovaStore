...
|-----------|----------|----------------------|
| `JWT_KEY` | Секретный ключ для подписи JWT (мин. 32 символа) | Используется значение из `Jwt:Key` в `appsettings.json`, иначе **небезопасный** dev-ключ |
...
> **Важно:** В production обязательно задайте `JWT_KEY` через переменную окружения. Ключ должен быть длиной не менее 32 символов.
...
Тесты написаны с использованием **xUnit** и **Moq**. Покрытие включает:
- `AuthControllerTests` — регистрация, логин, валидация, конфликты, неверные credentials
- `CartControllerTests` — работа корзины
- `ProductsControllerTests` — CRUD товаров
